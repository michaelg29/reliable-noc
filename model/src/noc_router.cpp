...
void noc_router::setup_ctrl() {
    for (int i = 0; i < NOC_N_RDIR; ++i) {
        rdir_ctrls[i] = noc_router_rctrl(i, _x, _y, rports[i]->is_dummy_if());
    }
    for (int i = 0; i < NOC_N_WDIR; ++i) {
        wdir_ctrls[i] = noc_router_wctrl(i, _x, _y, false);
    }
}

void noc_router::read_port(noc_dir_e dir, noc_data_t& data, noc_link_ctrl_t& link_ctrl) {
    wdir_ctrls[(uint32_t)(dir)].read_output(data, link_ctrl);
}
    ...
    while (true) {
        ...
        for (i = 0; i < NOC_N_RDIR; ++i) {
            ...
            if (link_ctrl.ctrl) {
                ...
                if (!rdir_ctrls[i].write_input(data, link_ctrl)) {
                    // send backpressure
                }
            }
        }
        ...
        for (j = 0; j < NOC_N_WDIR; ++j) {
            out_port_taken[j] = !wdir_ctrls[j].can_write_output();
        }
        ...
        for (i = NOC_N_RDIR-1; i >= 0; --i) {
            for (j = 0; j < NOC_N_WDIR; ++j) {
                ...
                if (!out_port_taken[j] && rdir_ctrls[i].read_input((noc_dir_e)j, data, link_ctrl)) {
                    // write to output FIFO
                    wdir_ctrls[j].write_output(data, link_ctrl);
